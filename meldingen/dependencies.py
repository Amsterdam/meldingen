...
def melding_repository_item(
    repository: Annotated[MeldingRepository, Depends(melding_repository)],
) -> RepositoryItem[Melding]:
...
def source_repository(session: Annotated[AsyncSession, Depends(database_session)]) -> SourceRepository:
...
def melding_answer_create_action(
    answer_repository: Annotated[AnswerRepository, Depends(answer_repository)],
    melding_repository_item: Annotated[RepositoryItem[Melding], Depends(melding_repository_item)],
...
def melding_answer_delete_action(
    ...
    return MeldingAnswerDeleteAction(melding_repository_item, answer_repository)
...
def download_attachment_action(
    ...
    return DownloadAttachmentAction(melding_repository_item, attachment_repository, filesystem)
...
def melder_melding_delete_attachment_action(
    ...
    return MelderDeleteAttachmentAction(melding_repository_item, attachment_repository, filesystem)
...
def melding_output_factory(
    ...
    source_output_factory: Annotated[SourceOutputFactory, Depends(source_output_factory)],
) -> MeldingOutputFactory:
    return MeldingOutputFactory(
        location_output_transformer, classification_output_factory, label_output_factory, source_output_factory
    )
...
def melding_update_output_factory(
    ...
    return MeldingUpdateOutputFactory(
        location_output_transformer, classification_output_factory, label_output_factory, source_output_factory
    )
...
def form_io_question_component_factory() -> FormIoQuestionComponentFactory:
    return FormIoQuestionComponentFactory()
