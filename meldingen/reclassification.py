...
class Reclassifier(BaseReclassification[Melding, Classification]):
    ...
    def __init__(self, answer_purger: AnswerPurger, asset_purger: AssetPurger) -> None:
        self._purge_answers = answer_purger
        self._purge_assets = asset_purger

    async def __call__(
        self, melding: Melding, old_classification: Classification | None, new_classification: Classification | None
    ) -> None:
